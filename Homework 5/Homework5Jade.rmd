...
```
Draw  two  new  sets  of  1000  bootstrap  samples  using  the  93  observations  and  report  the  95%  
bootstrap confidence intervals for each (do NOT plot the two new bootstrap distributions and do 
NOT use set.seed). Do these intervals support what you wrote for part (f)? Explain. 

#### Part h)

If you drew smaller sized bootstrap samples and calculated the 95% bootstrap confidence interval, 
how do you think the new intervals would compare with the interval reported in part (d)? Answer 
this before doing part (i) below. This question will only be graded for effort. 
...
```
Use set.seed(12) and draw three smaller sets (𝑛𝑛= 500, 100, and 10)  of bootstrap samples 
using the 93 observations (before to reset the seed before each bootstrap draw). Report the 95% 
bootstrap  confidence  intervals  for  each  of  these  new  sets.  Do  these  intervals  support  what  you 
