...
#### Part a)
```{r scatterplot}
h1 <- ggplot(Pdat2, aes(x=Pdat2$BetaDiet, y=Pdat2$BetaPlasma)) +
...
```{r scatterplot2}
h2 <- ggplot(Pdat2, aes(x=BetaDiet, y=log(BetaPlasma))) + 
...
```{r scatterplot3}
h3 <- ggplot(Pdat2, aes(x=log(BetaDiet), y=BetaPlasma)) + 
...
```{r scatterplot4}
h4 <- ggplot(Pdat2, aes(x=log(BetaDiet), y=log(BetaPlasma))) + 
  geom_point(size=1, shape=19) + xlab(" Natural Log Dietary Beta-Carotene (mcg/day") +
  ylab("Natural Log Plasma Beta-carotene (ng/ml)")
  ggarrange(h1, h2, h3, h4 + rremove("x.text"),
          ncol = 2, nrow = 2)
...
Beta Plasma would be at about 200 on the original BetaDiet v. BetaPlasma plot and regression line. s
